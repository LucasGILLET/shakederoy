...
import { Search, Sparkles, Flame, LogOut, User as UserIcon, Heart } from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-20">
          ...
          <div className="hidden md:flex items-center space-x-6">
            ...
            {user && (
              <Link href="/favorites" className="font-bold text-brand-dark hover:text-brand-primary transition-colors text-lg flex items-center gap-1 hover-scale">
                <Heart className="w-5 h-5" />
                Favoris
              </Link>
            )}
            {user?.role === 'admin' && (
              <Link href="/admin" className="font-bold text-brand-dark hover:text-brand-primary transition-colors text-lg hover-scale">
                Admin
              </Link>
            )}
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <Link href="/create" className="hidden sm:block">
              <Button variant="primary" size="sm">Creer</Button>
            </Link>
          </div>
        </div>
      </div>
