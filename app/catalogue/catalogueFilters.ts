...
function parseDifficulty(value: unknown): Cocktail['difficulty'] {
  ...
  if (normalizedValue.includes('facile') || normalizedValue.includes('easy')) {
    return 'Facile';
  }

  if (normalizedValue.includes('difficile') || normalizedValue.includes('hard')) {
    return 'Difficile';
  }
...
export function applyCatalogueFilters(
  ...
  const filtered = cocktails.filter((cocktail) => {
    ...
    if (
      ...
      cocktail.difficulty !== filters.difficultyFilter
    ) {
      return false;
    }
