...
export default function Catalogue() {
  ...
  useEffect(() => {
    ...
    const mergeFavoriteIds = (target: Set<string>, incoming: Set<string>) => {
      incoming.forEach((id) => target.add(id));
    };
    ...
    const fetchAllFavoriteIds = async (token: string | null): Promise<Set<string>> => {
      ...
      for (
        let requestCount = 0;
        requestCount < MAX_FAVORITES_PAGES;
        requestCount += 1
      ) {
        const endpoint =
          currentPage === 1
            ? '/cocktails/favorites'
            : `/cocktails/favorites?page=${currentPage}`;

        const pageData = await apiFetch<unknown>(endpoint, { headers });
        mergeFavoriteIds(allFavoriteIds, extractFavoriteIds(pageData));
    ...
    const fetchCocktails = async () => {
      try {
        ...
        const favoritesPromise = fetchAllFavoriteIds(token).catch((favoritesError) => {
          console.warn('Failed to fetch favorites:', favoritesError);
        ...
        setError(
          'Impossible de charger les cocktails. Veuillez réessayer plus tard.',
        );
      } finally {
        setLoading(false);
      }
  ...
  const filteredCocktails = useMemo(
    ...
    [cocktails, filterState, favoriteIds],
  );

  const activeFilterLabels = useMemo(
    () => getActiveFilterLabels(filterState),
    [filterState],
  );
          ...
          <div className="flex flex-col xl:flex-row gap-6 xl:items-end">
            <div className="flex-1 w-full">
              <label
                ...
                className="font-bold text-xl uppercase mb-4 block ml-2">
          ...
          {showFilters && (
            <div id="catalogue-filters-panel" className="mt-8 space-y-6">
              <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
                ...
                <div className="border-4 border-brand-dark bg-white p-5 shadow-hard">
                  <p className="font-black uppercase text-sm mb-4">Difficulté</p>
                  <div className="space-y-2">
              ...
              {hasAnyActiveFilters && (
                <div className="flex flex-wrap gap-2">
                  {activeFilterLabels.map((label) => (
                    <span
                      key={label}
                      className="px-4 py-2 border-2 border-brand-dark bg-white font-bold text-sm">
                      {label}
                    </span>
                  ))}
                </div>
        ...
        {!loading && filteredCocktails.length === 0 && !error && (
          <div className="text-center py-32 border-8 border-dashed border-gray-300 rounded-3xl">
            ...
            <p className="text-xl text-gray-500 mb-8">
              Essaie d&apos;élargir les filtres ou invente ta propre recette !
            </p>
            {hasAnyActiveFilters && (
            ...
            <div className="mt-6">
              <Link href="/create">
                <Button size="lg" className="shadow-hard">
                  Créer mon cocktail <Sparkles className="w-6 h-6 ml-2" />
                </Button>
              </Link>
            </div>
          </div>
